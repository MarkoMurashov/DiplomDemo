...
namespace DiplomDemo.Business_logic
{
    ...
    {
        public List<int> Execute(List<Instance> dataset, double alpha = 0.2)
        {
            var initialRoute = GreedyAlgorithm(dataset);

            Console.WriteLine("Result:");
            foreach (var point in initialRoute)
            {
                Console.Write($"{point}\t");
            }
            Console.WriteLine("Dist   " + CalculateDistanceValueForAllRoute(dataset, initialRoute));
            ...
            while (k < 5)
            {
                for (int i = 1; i < initialRoute.Count - 2; i++)
                {
                    for (int j = i + 2; j < initialRoute.Count; j++)
                    {
